...
pid_file="/tmp/tmux_mode_watcher_${socket_path////_}.pid"
...
trap 'rm -f "$pid_file" "${pid_file}.$$"' EXIT
...
# We track modes per session. client_prefix is per-client, so if any client
# attached to the session is in prefix mode, we report prefix for that session.
update_session_mode() {
    local session_id=$1
    local target="${session_id}:"

    local client_prefix pane_in_mode pane_synchronized
    client_prefix=$(tmux display-message -p -t "$target" '#{?client_prefix,1,0}' 2>/dev/null || true)
    pane_in_mode=$(tmux display-message -p -t "$target" '#{?pane_in_mode,1,0}' 2>/dev/null || true)
    pane_synchronized=$(tmux display-message -p -t "$target" '#{?pane_synchronized,1,0}' 2>/dev/null || true)

    local mode prompt style
    ...
    local env_var="tmux_mode_${session_id}"

    # Check if already correct to avoid needless refresh
    local current_val
    current_val=$(tmux show-environment -t "$session_id" "$env_var" 2>/dev/null | sed 's/^[^=]*=//' || true)
    if [ "$current_val" != "$mode" ]; then
        tmux set-environment -t "$session_id" "$env_var" "$mode" 2>/dev/null || true

        # Refresh all clients attached to this session so the update is immediate
        tmux list-clients -t "$session_id" -F '#{client_tty}' 2>/dev/null | while IFS= read -r client_tty; do
...
while true; do
    tmux list-sessions -F '#{session_id}' 2>/dev/null | while IFS= read -r session_id; do
        ...
        update_session_mode "$session_id"
    done
